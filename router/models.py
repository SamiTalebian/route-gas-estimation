...
class GasStationRoute(models.Model):
    ...
    def find_cheapest_fuel_station(self, point, distance_limit=15, file_path="fuel-prices-with-coordinates.csv"):
    ...
    def update_new_location(self, cheapest_station):
        self.latitude_start = cheapest_station['Coordinates'][0]
        self.longitude_start = cheapest_station['Coordinates'][1]
        self.save()

    def fill_gas(self, point, distance_limit=10, file_path="fuel-prices-with-coordinates.csv"):
        station_point = self.find_cheapest_fuel_station(point, distance_limit, file_path)
        self.update_new_location(station_point)
        return station_point['Retail Price']

    def check_gas_needed(self, point, limit=790): # 500 miles -> 804 km
    ...
    def vehicle_move_function(self, route):
        ...
        for point in geometry_points:
            if self.check_gas_needed(point):
                price = self.fill_gas(point)
                self.add_gas_price_to_cost(price)
    ...
    def add_gas_price_to_cost(self, price):
